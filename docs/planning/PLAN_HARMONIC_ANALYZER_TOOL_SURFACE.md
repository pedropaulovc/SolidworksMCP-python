...
- **#4 — `feat(adapter): implement sweep and loft COM operations`.** This
...
- **#5 — `feat(adapter): implement missing sketch primitive COM calls`.**
  **Hard prerequisite for this entire plan.** Five sketch primitives
  ...
  20-channel layout needs `sketch_circular_pattern`). **Issue #5 must
  land before Phase 1 of this plan is usable end-to-end.**
- **#6 — `fix(service): replace mocked interference check`.** The
  ...
  #6 landing first when driven through the UI checkpoint executor.
  Direct MCP tool calls are unaffected.

**Strategic complementarity — these issues become more valuable as this plan ships:**

- **#20 — `feat(soc): SolidWorks as Code`.** The Python-script artifact
...
- **#8 — `feat(agents): SQLite workflow database MVP`.** The session
...
- #7 (file management safety), #9/#10 (UI events), #12/#13 (agent
...
replacing it. The recommended landing order is: **#5 → #4 → Phase 0-6
of this plan (some sub-phases of which directly close #4 and contribute
to #20)**.
...
## Phase 0 — Wire existing stubs (closes upstream #4)

Estimated effort: 1 day.

This phase is identical in scope to upstream issue #4
...
- [ ] Implement `create_sweep` adapter method — open sketch as profile
  ...
  (per issue #4). Twist params optional. Follow the `create_revolve`
...
Verification (matches issue #4 acceptance criteria): no-model guard
...
**Depends on upstream issue #5.** The sketch primitives the harmonic
...
also requires upstream #6 to land so `check_interference` is no
...
1. **Sequencing relative to existing issues.** Issue #5 is a hard
   prerequisite for the consumer project. Should Phase 0 of this plan
   wait on #5, ship in parallel, or land first as an independent
   contribution? Proposing parallel: Phase 0 (sweep/loft, closes #4)
   is independent of #5 and unblocks separate workflows.
...
6. **SoC integration (#20).** Should each new tool include a
